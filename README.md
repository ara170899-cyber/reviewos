...
## Известные ограничения WB API

WB Seller API (`feedbacks-api.wildberries.ru`) имеет **очень строгий накопительный rate-limit**:

- ~1 запрос/сек на токен
- 2 POST'а к `/feedbacks/answer` в 30 секунд → **бан 5-15 минут**
- Серия бурстов → бан **на часы**
- Бан **по `sid` сессии**, не по конкретному JWT — пересоздание токена с тем же session id **не сбрасывает** бан

**Что мы сделали**: серверный дроссель `wbThrottle()` — GET 10 сек между запросами, POST/PATCH **45 сек**. Запросы не падают, а **встают в очередь**. Это даёт безопасный auto-pilot, но между публикациями двух ответов будет естественная **~45-секундная пауза**.

**Что юзер должен знать**:
1. Между сохранением WB-ключа и первым cycle — **warmup 15 минут** (мы не дёргаем токен сразу, даём ему «отдохнуть» от возможных внешних лимитов).
2. Если WB всё-таки забанил — `data/wb-bans.json` хранит таймер. Cycle пропускает WB пока бан активен.
3. Для разработки и тестов используйте **sandbox** (`Settings → 🧪 Sandbox-режим`) — отдельная среда WB без production-банов.
4. WB-баны **не блокируют** Ozon: отзывы/вопросы/чаты Ozon работают параллельно.

## Стоимость AI

Каждый AI-ответ — это вызов Anthropic Claude или OpenRouter (платный):

| Провайдер · модель | Примерная цена 1 ответа |
|---|---|
| Anthropic Claude Haiku 4.5 | ~$0.001–0.003 |
| Anthropic Claude Sonnet 4.6 | ~$0.005–0.015 |
| OpenRouter GPT-4o-mini | ~$0.0005 |
| OpenRouter GPT-4o | ~$0.005–0.020 |

Для 100 отзывов в день на Haiku — около **$3–9/месяц**. На GPT-4o-mini — **$1.5/месяц**. Выбирай модель в Settings → 🤖 AI-провайдер.
